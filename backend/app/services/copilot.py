...
SYSTEM_PROMPT = """你是"本体智能副驾"，一个面向本体建模与本体驱动策略推理的 AI 助手。

你的能力：
1. 解释本体模型中的实体、属性、关系
2. 分析业务场景和数据资产
3. 推荐策略并生成推理链（本体查询 → 计算/预测 → 策略输出）
4. 回答关于当前本体的建模问题
...
def _prepare_messages(messages: list[dict], db: Session, entity_id: str | None = None) -> list[dict]:
    """构造发往 LLM 的消息列表：注入本体上下文 system prompt + 拼接历史消息。"""
...
def chat_stream(messages: list[dict], db: Session, entity_id: str | None = None):
    ...
    api_messages = _prepare_messages(messages, db, entity_id)
