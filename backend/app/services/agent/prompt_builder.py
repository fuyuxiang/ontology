...
def build_system_prompt(
    ...
    return (
        ...
        "```\n"
        "\n"
        "### 场景D：宽带装机退单稽核类问题（如'退单根因分布？'、'施工原因退单有什么共性？'）\n"
        "```\n"
        "Step 1: 感知 · 数据采集 → query_entity_data 查询退单工单、客户、工程师、派单等关联实体数据\n"
        "Step 2: 识别 · 证据提取 → get_business_rules 获取退单稽核规则，结合 NLP 证据和规则证据进行命中判定\n"
        "Step 3: 推理 · 逻辑命中 → evaluate_all_rules 执行逻辑函数，计算各根因类别（用户原因/施工原因/资源原因/业务原因）的置信度\n"
        "Step 4: 归因 · 结论输出 → 综合置信度给出根因判定，输出稽核动作（如回访核实、资源派修、工程师培训等）\n"
        "```\n"
        "\n"
        "## 回答格式\n"
        "\n"
