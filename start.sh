...
# 冷启动本地 MySQL 约 18s；内网 MySQL + 26 个 workspace 函数全量扫描约 75-90s，
# 给到 100s 留余量，避免误判失败导致前端与 code-server 不启动。
backend_pids=""
...
if [ -z "$backend_pids" ]; then
    echo "Backend failed to start after 100s. Check backend.log"
    exit 1
fi
